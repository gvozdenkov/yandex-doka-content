...
Для лучшего понимания, как работает JS, лучше применять термин "идентификатор". `var x;` создаёт идентификатор `x` — это токен языка, который теперь хранится в памяти.
